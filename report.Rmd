...
The map below shows reported levels of `r tolower(filtered[1,"Indicator.Name"])` in `r input$yearInput` for all 50 states. The color of each state represents the relative number of `r tolower(filtered[1,"Indicator.Name"])` reported in `r input$yearInput` for each state.

The circles on each state represent the annual rate of change in `r tolower(filtered[1,"Indicator.Name"])` observed between 2013 and 2016. Green circles indicate states that are improving, and red circles indicate states that are getting worse. Large circles indicate a higher rate of change.
...
*Note*: The data presented here are based on The Multiple Cause of Death data available on CDC WONDER. They represent age-adjusted rates per 100,000 residents from the corresponding year.

Overall, the average rate of `r tolower(filtered[1,"Indicator.Name"])` across all 50 states was `r round(mean(filtered$value, na.rm = TRUE), digits = 1)` per 100,000 residents in `r input$yearInput`.
...
The graph below displays the predicted forecast for `r tolower(filtered[1,"Indicator.Name"])` based on Multiple Cause of Death data from 2013 to 2016.
...
Across all years surveyed, the state with the lowest level of `r tolower(filtered[1,"Indicator.Name"])` was `r filtered[which.min(filtered$value),"State"]` in `r filtered[which.min(filtered$value),"variable"]` with a rate of `r round(filtered[which.min(filtered$value),"value"],1)`.

The state with the highest level of `r tolower(filtered[1,"Indicator.Name"])` was `r filtered[which.max(filtered$value),"State"]` in `r filtered[which.max(filtered$value),"variable"]` with a rate of `r round(filtered[which.max(filtered$value),"value"],1)`.

Overall, `r newslope[which.max(newslope$Slope),"State"]` had the highest rate of change with an average annual change of `r round(newslope[which.max(newslope$Slope),"Slope"],1)`.

\newpage
A table of the data used to generate this report is included below.

``` {r echo=FALSE, warning=FALSE}
    short <- filter(data, Indicator.Number == input$indicatorInput)
    #change state names to lower-case for merge
    showvars <- c("State","Year","Age.Adjusted.Rate")
    display <- short[showvars]
    library(knitr)
