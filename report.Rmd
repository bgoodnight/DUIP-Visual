...
###State and nationwide trends for `r tolower(filtered[1,"Indicator.Name"])`

The map below shows reported levels of `r tolower(filtered[1,"Indicator.Name"])` in `r input$yearInput` for all PfS / DDPI states.

The color of each state represents the level of `r tolower(filtered[1,"Indicator.Name"])` in `r input$yearInput`.
The symbols on each state represent the yearly change in the annual rate of `r tolower(filtered[1,"Indicator.Name"])` observed between 2013 and 2015.
...
The average level of `r tolower(filtered[1,"Indicator.Name"])` in `r input$yearInput` for all PfS / DDPI states was `r round(mean(filtered$value, na.rm = TRUE), digits = 1)`.
...
Nationwide, the number of `r tolower(filtered[1,"Indicator.Name"])` has been `r slopesign` by `r round(regression$coeff[2], 1)` annually, on average.

If these trends continue, by 2018 the average level of `r tolower(filtered[1,"Indicator.Name"])` is predicted to be `r round(regression$coeff[1] + (regression$coeff[2]*2018),1)`.
